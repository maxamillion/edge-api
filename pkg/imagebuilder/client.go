...
type ImageBuilderClientInterface interface {
	...
	ComposeInstaller(updateTransactionord *models.UpdateTransaction, image *models.Image, headers map[string]string) (*models.Image, error)
...
func (c *ImageBuilderClient) ComposeInstaller(updateTransactionord *models.UpdateTransaction, image *models.Image, headers map[string]string) (*models.Image, error) {
	pkgs := make([]string, 0)
	req := &ComposeRequest{
		Customizations: &Customizations{
			Packages: &pkgs,
		...
		ImageRequests: []ImageRequest{
			{
				...
				Ostree: &OSTree{
					...
					URL: fmt.Sprintf("%s/%s/%d/repo", c.RepoURL, updateTransactionord.Account, updateTransactionord.ID),
				},
				UploadRequest: &UploadRequest{
					Options: make(map[string]string),
					Type:    "aws.s3",
